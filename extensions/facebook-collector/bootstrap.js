...
function createMessageListener(state) {
  return (event) => {
    ...
    if (event.data?.type === "FLIP_COLLECTOR_VALIDATE_REQUEST") {
      recordBootstrapBeacon("PAGE_MESSAGE_RECEIVED", requestId, eventType);
      void sendRuntime({ type: "VALIDATE_COLLECTOR", requestId }).then((result) => respond(event.origin, "FLIP_COLLECTOR_VALIDATE_RESULT", requestId, result)).catch((error) => { if (!handleRuntimeFailure(state, error)) respond(event.origin, "FLIP_COLLECTOR_VALIDATE_RESULT", requestId, { ok: false, error: "COLLECTOR_VALIDATION_FAILED" }); });
      return;
    }

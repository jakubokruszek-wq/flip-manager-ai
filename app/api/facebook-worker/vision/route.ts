...
export async function POST(request: Request) {
  ...
  try {
    ...
    const vision = await analyzeFacebookImages([input.screenshotDataUrl]);
...
function parsePayload(value: unknown): { jobId: string; leaseToken: string; workerId: string; postId: string; screenshotDataUrl: string } {
  ...
  return { jobId, leaseToken, workerId, postId, screenshotDataUrl };
}

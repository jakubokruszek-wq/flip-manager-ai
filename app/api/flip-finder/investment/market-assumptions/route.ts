...
export async function GET(): Promise<Response> {
  try {
    ...
    console.error("MARKET ASSUMPTIONS GET ERROR", error);
    return Response.json({ message: "Nie udało się pobrać założeń rynku." }, { status: 500 });
  }
}
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    console.error("MARKET ASSUMPTIONS POST ERROR", error);
    return Response.json({ message: "Nieprawidłowe założenie rynku." }, { status: 400 });
  }
}

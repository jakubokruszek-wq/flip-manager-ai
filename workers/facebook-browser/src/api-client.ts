...
export function createFacebookApiClient(config: FacebookWorkerConfig) {
  ...
  return {
    ...
    postCache: (job: FacebookWorkerJob, postIds: string[], signal?: AbortSignal) => post<{ hits: Record<string, FacebookPostCacheHit & { publishedAt: string }>; ageHits: Record<string, FacebookAgeCacheHit> }>("/api/facebook-worker/post-cache", { jobId: job.id, leaseToken: job.leaseToken, workerId: config.workerId, postIds }, signal),

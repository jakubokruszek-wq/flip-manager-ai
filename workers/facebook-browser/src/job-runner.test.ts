  ...
  const result = await runFacebookJobCompletion(async () => {
    ...
    for (const post of discovery.posts) {
      ...
      posts.push(await processDedicatedFacebookPost(post, "group-1", {
        ...
        capture: async () => ({ screenshotDataUrl: "data:image/jpeg;base64,AA==", imageUrls: [], mediaCandidates: [], publishedAt: null, authoritativePostText: "", authoritativePostTextSource: "NONE", authoritativePostTextProvenance: "NONE", box: { x: 0, y: 0, width: 500, height: 400 }, candidateCount: 1, screenshotWidth: 500, screenshotHeight: 400, captureMethod: "ELEMENT_SCREENSHOT", compressed: false }),

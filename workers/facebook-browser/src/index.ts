      ...
      if (job) {
        ...
        try {
          const result = await runFacebookJobCompletion(
            () => fetchFacebookGroupWithBrowser(config.profileDir, job.groups[0], shutdown.signal, async (input, signal) => (await api.vision(job, input, signal)).vision, async () => { await api.heartbeat(job, shutdown.signal); }, timeDiagnosticMode, debugMaxPosts, mediaDiagnosticMode, debugPostId),

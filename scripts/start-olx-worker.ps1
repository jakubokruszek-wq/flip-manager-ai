...
$LogPath = Join-Path $LogDirectory "olx-worker.log"
...
while ($true) {
    $startedAt = Get-Date
    Write-SupervisorLog "Starting OLX worker."
    $exitCode = 1

    try {
        & npm.cmd run olx-worker 2>&1 | ForEach-Object {
    ...
    Write-SupervisorLog "OLX worker exited with code $exitCode."
    $now = Get-Date

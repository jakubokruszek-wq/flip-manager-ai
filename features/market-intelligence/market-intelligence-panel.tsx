...
function ResaleCompsSection({ data }: { data: MarketIntelligence }) {
  ...
  return <section className="space-y-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.04] p-4 sm:p-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Metric label="Mediana / m²" value={perSqm(data.resaleCompMedianPricePerSqm ?? null)} /><Metric label="Średnia ważona / m²" value={perSqm(data.resaleCompWeightedPricePerSqm ?? null)} /><Metric label="Ostrożny" value={currency(data.resaleCompLowPrice ?? null)} /><Metric label="Bazowy" value={currency(data.resaleCompExpectedPrice ?? null)} /><Metric label="Optymistyczny" value={currency(data.resaleCompHighPrice ?? null)} /></div>
    <div className="grid gap-3 sm:grid-cols-2"><Metric label="Rekomendowana cena wystawienia" value={currency(data.recommendedListingPrice ?? null)} /><Metric label="Szacowana cena sprzedaży" value={currency(data.estimatedSalePrice ?? null)} description="z uwzględnieniem negocjacji" /></div>

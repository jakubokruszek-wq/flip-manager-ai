...
export async function repairFacebookGalleryJob(listingId: string): Promise<{ jobId: string | null; status: FacebookGalleryStatus; listingId: string; created?: boolean }> {
  ...
  if (result.error || !result.data) {
    ...
    const knownCode = ["FACEBOOK_GALLERY_LISTING_NOT_FOUND", "FACEBOOK_GALLERY_LISTING_NOT_ELIGIBLE", "FACEBOOK_GALLERY_EXACT_POST_REQUIRED", "FACEBOOK_GALLERY_FILTER_CONTEXT_MISSING", "FACEBOOK_GALLERY_REPAIR_ALREADY_RUNNING", "FACEBOOK_GALLERY_METADATA_IDENTITY_AMBIGUOUS", "FACEBOOK_GALLERY_METADATA_GROUP_MISMATCH"].find((code) => message.includes(code));
    throw new Error(knownCode ?? `FACEBOOK_GALLERY_REPAIR_FAILED: ${message}`);
  }
  const value = row(result.data);
  const jobId = string(value?.gallery_job_id);
  const postId = string(value?.expected_post_id);
  if (!value || !jobId || !postId) throw new Error("FACEBOOK_GALLERY_REPAIR_FAILED: invalid result");
  return { jobId, status: "PENDING", listingId: String(value.listing_id ?? listingId), created: true };
}

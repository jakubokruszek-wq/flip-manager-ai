...
import type { CanonicalDeal, DealFactOverrides, DealListingInput, DirectorTrackRecord, MarketEvidence } from "../types";
...
async function persistIntelligence(db: ReturnType<typeof createAdminClient>, deal: CanonicalDeal): Promise<void> {
  if (deal.evidenceFabric.length) {
    const evidenceRows = deal.evidenceFabric.map((item) => ({ id: item.id, deal_id: item.dealId, type: item.type, source_type: item.sourceType, source_name: item.sourceName.slice(0, 160), structured_payload: item.value, source_url: item.sourceUrl, document_id: item.documentId, observed_at: item.observedAt, valid_from: item.validFrom, valid_until: item.validUntil, reliability: item.reliability, confidence: item.confidence, director_who_requested: item.directorWhoRequested, verification_status: item.verificationStatus, conflicts_with: item.conflictsWith }));
    const { error } = await db.from("deal_evidence").upsert(evidenceRows, { onConflict: "id" }); if (error) throw error;
  }
  const directors = [deal.scout, deal.verify, deal.market, deal.underwriting, deal.ceo];
  const runRows = directors.map((output) => ({ deal_id: deal.id, director: output.director, input_fingerprint: output.inputFingerprint, output_version: output.version, status: output.status, tools_requested: output.execution.toolsRequested, tools_succeeded: output.execution.toolsSucceeded, tools_failed: output.execution.toolsFailed, evidence_count: output.execution.evidenceCount, conflict_count: output.execution.conflictCount, output, confidence: output.confidence, elapsed_ms: output.execution.elapsedMs, computed_at: output.computedAt }));
  const { error: runError } = await db.from("director_runs").upsert(runRows, { onConflict: "deal_id,director,input_fingerprint,output_version" }); if (runError) throw runError;

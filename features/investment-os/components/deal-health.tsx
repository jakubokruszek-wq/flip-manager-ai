...
import { ConfidenceMeter, formatInvestmentRisk, MetricTile, SectionHeading } from "./investment-ui";
...
export function DealHealth({ deal }: { deal: CanonicalDeal }) {
  ...
  return <section aria-labelledby="deal-health-title" className="min-w-0 max-w-full space-y-3">
    <SectionHeading eyebrow="02 · ekonomika i pewność" id="deal-health-title" title="Stan analizy" aside={blocked.length ? <span className="rounded-full border border-warning/35 bg-warning/10 px-3 py-1 text-xs font-semibold text-warning">Bramka zakupu zablokowana · {blocked.length}</span> : null} />
    {blocked.length ? <div className="rounded-xl border border-amber-500/30 bg-amber-500/[0.07] px-4 py-3 text-sm">
      ...
      <p className="mt-1 text-muted-foreground">{blocked.slice(0, 2).map((gate) => formatInvestmentRisk(gate.reason)).join(" · ")}{blocked.length > 2 ? ` · oraz ${blocked.length - 2} kolejnych` : ""}</p>
    </div> : null}
    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <MetricTile label="Odsprzedaż · bazowa" value={money(market?.resaleValueBase)} detail={market ? `${money(market.resalePricePerM2Base)} / m²` : undefined} />

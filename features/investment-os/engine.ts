...
import type { BuildDealInput, CanonicalDeal, CeoResult, DealFacts, DealPlaybook, DirectorOutput, FactValue, MarketResult, ProvenanceEntry, VerifyResult } from "./types";
...
export function buildCanonicalDeal(input: BuildDealInput): CanonicalDeal {
  ...
  const playbook = buildPlaybook(facts, ceo);
  ...
  return { id: input.dealId, listingId: input.listing.id, stage, factsFingerprint, facts, scout, verify, market, underwriting, ceo, playbook, createdAt: input.createdAt ?? input.now, updatedAt: input.now };
}
...
function buildCeo(input: BuildDealInput, verify: DirectorOutput<VerifyResult>, market: DirectorOutput<MarketResult>, underwriting: ReturnType<typeof buildUnderwriter>): DirectorOutput<CeoResult> {
  ...
  if (!underwriting.result) {
    const rejected = input.listing.manualDecision === "REJECTED";
    const action = rejected ? "ODRZUĆ" : "HOLD / ZBIERZ DANE";
    return output("CEO", fp, input.now, Math.min(verify.confidence, market.confidence), { decision: rejected ? "REJECT" : "REVIEW", action, headline: rejected ? "Oferta odrzucona ręcznie" : "Uzupełnij dane przed decyzją", openingOffer: null, targetPurchasePrice: null, maxPurchasePrice: null, expectedProfitBase: null, expectedProfitConservative: null, flipScore: 0, confidence: Math.min(verify.confidence, market.confidence), strengths: [], risks: [...verify.warnings, ...market.warnings], missingBeforeViewing: verify.result?.missingCriticalFields ?? [], missingBeforePurchase: verify.result?.missingOptionalFields ?? [], recommendation: action, investmentThesis: rejected ? "Manualna decyzja zamyka analizę inwestycyjną." : "Nie ma wystarczających danych rynkowych do wiarygodnej tezy inwestycyjnej.", bearCase: "Nie można policzyć ostrożnego wyniku bez wiarygodnego ARV.", baseCase: "Scenariusz bazowy pozostaje zablokowany.", bullCase: "Scenariusz optymistyczny nie jest podstawą do decyzji bez evidence.", dissent: rejected ? [] : ["Potencjał oferty może istnieć, ale nie został jeszcze udowodniony."], conditionsToProceed: [...(verify.result?.missingCriticalFields ?? []), ...market.missingFields].map((field) => `Potwierdź: ${field}`), walkAwayConditions: rejected ? ["Manual REJECT"] : ["Brak wiarygodnego założenia resale po uzupełnieniu danych"], nextBestAction: rejected ? "Nie reaktywuj automatycznie." : "Uzupełnij brakujące dane i uruchom przeliczenie." }, [...verify.missingFields, ...market.missingFields], [...verify.warnings, ...market.warnings], [rejected ? "CEO_MANUAL_REJECT" : "CEO_DEPENDENCY_BLOCKED"], [], rejected ? "COMPLETE" : "BLOCKED");
  }
  ...
  const result: CeoResult = { decision, action, headline: decision === "TOO_EXPENSIVE" ? "Dobry profil, ale cena wymaga negocjacji" : decision === "REVIEW" ? "Potencjał wymaga potwierdzenia danych" : decision === "REJECT" ? "Twardy warunek lub decyzja ręczna wyklucza ofertę" : decision === "HOT" ? "Mocny deal w granicach opłacalności" : "Warto umówić oględziny", openingOffer, targetPurchasePrice: u.targetPurchasePrice, maxPurchasePrice: u.maxPurchasePrice, expectedProfitBase: u.profitBase, expectedProfitConservative: u.profitLow, flipScore: u.flipScore, confidence: Math.min(u.confidenceScore, verify.confidence), strengths: u.strengths, risks: u.redFlags, missingBeforeViewing: verify.result?.missingCriticalFields ?? [], missingBeforePurchase: verify.result?.missingOptionalFields ?? [], recommendation, investmentThesis: u.profitBase === null ? "Ekonomia nie została policzona." : `Scenariusz bazowy daje ${money(u.profitBase)} zł zysku przed podatkiem dochodowym przy maksymalnym zakupie ${u.maxPurchasePrice == null ? "nieustalonym" : `${money(u.maxPurchasePrice)} zł`}.`, bearCase: scenarioCase("Ostrożny", u.scenarios.conservative.resaleValue, u.profitLow), baseCase: scenarioCase("Bazowy", u.scenarios.base.resaleValue, u.profitBase), bullCase: scenarioCase("Optymistyczny", u.scenarios.optimistic.resaleValue, u.profitHigh), dissent: buildDissent(u, verify, market), conditionsToProceed: [u.maxPurchasePrice == null ? null : `Cena zakupu nie wyższa niż ${money(u.maxPurchasePrice)} zł`, ...(verify.result?.missingCriticalFields ?? []).map((field) => `Potwierdź ${field}`)].filter((value): value is string => Boolean(value)), walkAwayConditions: [u.maxPurchasePrice == null ? "Brak wiarygodnej ceny maksymalnej" : `Sprzedający nie schodzi do ${money(u.maxPurchasePrice)} zł`, "Potwierdzona wada prawna lub twarde wykluczenie filtra"], nextBestAction: action === "NEGOCJUJ" ? `Zacznij od ${openingOffer == null ? "oferty warunkowej" : `${money(openingOffer)} zł`}.` : action === "JEDŹ OBEJRZEĆ" || action === "KUP" ? "Umów oględziny i potwierdź checklistę przed zakupem." : action === "ODRZUĆ" ? "Zamknij deal bez automatycznej reaktywacji." : "Uzupełnij brakujące dane." };
...
function output<T>(director: DirectorOutput<T>["director"], inputFingerprint: string, computedAt: string, confidence: number, result: T | null, missingFields: string[], warnings: string[], reasonCodes: string[], provenance: ProvenanceEntry[], status: DirectorOutput<T>["status"] = "COMPLETE"): DirectorOutput<T> {
  const missing = unique(missingFields); const risks = unique(warnings); const reasons = unique(reasonCodes);
  return { director, status, version: 1, inputFingerprint, computedAt, confidence: clamp(confidence, 0, 100), result, missingFields: missing, warnings: risks, reasonCodes: reasons, provenance,
    finding: directorFinding(director, status, confidence, missing), recommendation: directorRecommendation(director, status, missing), evidence: provenance.map((item) => `${item.field}:${item.provenance}${item.sourceId ? `:${item.sourceId}` : ""}`), risks, missingData: missing, nextBestActions: missing.length ? missing.map((field) => `Potwierdź ${field}`) : ["Utrzymaj wynik aktualny przy zmianie danych"], decisionTriggers: reasons, whatWouldChangeMyMind: missing.length ? missing.map((field) => `Nowe wiarygodne dane dla ${field}`) : ["Zmiana materialnego faktu lub założenia"] };
}

function buildPlaybook(facts: DealFacts, ceo: DirectorOutput<CeoResult>): DealPlaybook {
  const missingViewing = ceo.result?.missingBeforeViewing ?? []; const missingPurchase = ceo.result?.missingBeforePurchase ?? [];
  return {
    beforeCall: ["Otwórz kanoniczne ogłoszenie i historię ceny", ...missingViewing.map((field) => `Przygotuj pytanie o ${field}`)],
    sellerQuestions: ["Jaki jest stan prawny i forma własności?", "Dlaczego nieruchomość jest sprzedawana?", "Jak długo oferta jest aktualna?", "Czy cena podlega negocjacji?", ...(facts.monthlyFee.effectiveValue == null ? ["Jaki jest czynsz i co obejmuje?"] : [])],
    viewingChecklist: ["Potwierdź metraż i układ", "Sprawdź instalacje, wilgoć, piony i wentylację", "Zweryfikuj stan klatki i części wspólnych", "Zapisz realny zakres remontu", ...missingViewing.map((field) => `Zweryfikuj ${field}`)],
    negotiationPlan: [ceo.result?.openingOffer == null ? "Złóż ofertę warunkową po uzupełnieniu danych" : `Pierwsza oferta: ${money(ceo.result.openingOffer)} zł`, ceo.result?.targetPurchasePrice == null ? "Ustal target po analizie" : `Cel: ${money(ceo.result.targetPurchasePrice)} zł`, ceo.result?.maxPurchasePrice == null ? "Nie deklaruj maksimum bez pełnego underwriting" : `Nie przekraczaj: ${money(ceo.result.maxPurchasePrice)} zł`],
    documentsRequired: ["Księga wieczysta lub dokument prawa do lokalu", "Zaświadczenie o braku zaległości", "Rzut lokalu", "Dokumenty wspólnoty/spółdzielni", "Informacja o funduszu remontowym"],
    conditionsBeforePurchase: [...missingPurchase.map((field) => `Potwierdź ${field}`), ...(ceo.result?.conditionsToProceed ?? [])],
  };
}

function scenarioCase(label: string, resale: number | null, profit: number | null): string { return `${label}: sprzedaż ${resale == null ? "nieustalona" : `${money(resale)} zł`}, zysk ${profit == null ? "nieustalony" : `${money(profit)} zł`}.`; }
function buildDissent(u: NonNullable<DirectorOutput<ReturnType<typeof calculateUnderwriting>>["result"]>, verify: DirectorOutput<VerifyResult>, market: DirectorOutput<MarketResult>): string[] { const values: string[] = []; if (u.flipScore >= 70 && Math.min(verify.confidence, market.confidence) < 70) values.push("Wysoki potencjał ekonomiczny opiera się na danych o niskiej pewności."); if ((u.profitLow ?? 0) < 0 && (u.profitBase ?? 0) > 0) values.push("Scenariusz ostrożny jest stratny mimo dodatniej bazy."); if (u.decision === "TOO_EXPENSIVE") values.push("Nieruchomość może być właściwa, ale obecna cena niszczy wymagane progi."); return values; }
function directorFinding(director: DirectorOutput<unknown>["director"], status: DirectorOutput<unknown>["status"], confidence: number, missing: string[]): string { return status === "BLOCKED" ? `${director} zablokowany przez brak wymaganych danych.` : `${director} zakończył analizę z confidence ${Math.round(confidence)}${missing.length ? ` i ${missing.length} brakami` : ""}.`; }
function directorRecommendation(director: DirectorOutput<unknown>["director"], status: DirectorOutput<unknown>["status"], missing: string[]): string { if (status === "BLOCKED") return `Uzupełnij ${missing.join(", ") || "wejście"}.`; if (director === "SCOUT") return "Przekaż kanoniczne fakty do VERIFY."; if (director === "VERIFY") return missing.length ? "Potwierdź brakujące fakty przed zakupem." : "Przekaż zweryfikowane fakty do MARKET."; if (director === "MARKET") return "Użyj zakresu LOW/BASE/HIGH w underwriting."; if (director === "UNDERWRITER") return "Przestrzegaj max purchase i progów rentowności."; return "Wykonaj next best action CEO."; }

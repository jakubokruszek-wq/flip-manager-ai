...
import type { FacebookGroupAccessStatus, FacebookGroupInput, WatchedFacebookGroup } from "./types";
...
function validateUrl(value: string) { const url = new URL(value); if (url.protocol !== "https:" || !/(^|\.)facebook\.com$/i.test(url.hostname)) throw new Error("Podaj bezpieczny adres HTTPS grupy Facebook."); return url.toString(); }

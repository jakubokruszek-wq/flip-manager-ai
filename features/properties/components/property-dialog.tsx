      ...
      <DialogContent className="left-0 top-0 h-dvh max-h-none max-w-none translate-x-0 translate-y-0 gap-0 overflow-x-hidden overflow-y-auto rounded-none border-border/70 bg-card p-0 shadow-2xl shadow-black/20 sm:left-1/2 sm:top-1/2 sm:h-auto sm:max-h-[calc(100dvh-1.5rem)] sm:max-w-6xl sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-3xl">
        <header className="relative overflow-hidden border-b border-border/70 bg-gradient-to-br from-primary/[0.13] via-card to-card px-5 pb-6 pt-7 sm:px-8 sm:pb-8 sm:pt-10">
          ...
          <div className="relative flex flex-col gap-5 pr-8 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <p className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-semibold text-primary"><Sparkles className="size-3.5" />Panel inwestycji</p>
            ...
            <div className="ui-card p-4 text-center"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">Ocena inwestycji</p><p className="type-financial-hero mt-1 text-gold">{formatFlipScore(property.flipScore ?? score.score)}</p><p className="mt-1 text-xs font-semibold text-muted-foreground">{score.label}</p></div>
          </div>
        </header>

        {!property.listingId && <p className="mx-5 mt-4 text-sm text-muted-foreground" role="status">Brak powiązanego ogłoszenia. Deal Room nie jest dostępny dla tej nieruchomości.</p>}
        ...
        <div className="space-y-8 px-5 py-6 sm:px-8 sm:py-8">
          ...
          <div className="grid gap-8 xl:grid-cols-2">
            ...
            <DashboardSection icon={Sparkles} title="Ocena inwestycji">
          ...
          <DashboardSection icon={BrainCircuit} title="Ocena potencjału" subtitle={`Pewność analizy: ${analysis.confidence}%`}>
            <div className="rounded-2xl border border-gold/20 bg-gradient-to-br from-gold/[0.09] to-transparent p-5"><p className="leading-7 text-foreground/85">{analysis.summary}</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><AnalysisList label="Mocne strony" values={analysis.strengths} /><AnalysisList label="Ryzyka i rekomendacje" values={[...analysis.risks, ...analysis.recommendations]} /></div></div>
          </DashboardSection>
          ...
          <DashboardSection icon={TrendingUp} title="Analiza inwestycyjna" subtitle={investmentAnalysis ? `Wykonana: ${formatDate(investmentAnalysis.analyzedAt)}` : undefined}>
            {investmentAnalysis ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><InfoMetric label="Ocena inwestycji" value={formatFlipScore(investmentAnalysis.flipScore.score)} emphasis /><InfoMetric label="Decyzja zakupu" value={decisionLabel(investmentAnalysis.purchaseRecommendation.decision)} emphasis /><InfoMetric label="Maksymalna cena" value={formatCurrency(investmentAnalysis.purchaseRecommendation.recommendedMaxPrice)} /><InfoMetric label="Cel negocjacyjny" value={formatCurrency(investmentAnalysis.purchaseRecommendation.negotiationTarget)} /><InfoMetric label="Wycena po remoncie" value={formatCurrency(investmentAnalysis.marketIntelligence.estimatedAfterRenovationPrice)} /><InfoMetric label="Zysk" value={formatCurrency(investmentAnalysis.calculator.profit)} emphasis /><InfoMetric label="Zwrot z inwestycji (ROI)" value={formatPercent(investmentAnalysis.calculator.roi)} emphasis /><InfoMetric label="Pewność" value={`${investmentAnalysis.aiAnalysis.confidence}%`} /></div> : <EmptyState text="Analiza inwestycyjna nie została jeszcze zapisana. Dodaj ofertę z Flip Finder po wykonaniu analizy." />}
          </DashboardSection>
...
function PropertyHero({ property, score, title, location, onDeleted, onUpdated }: { property: PropertyWithInvestmentAnalysis; score: FlipScoreResult; title: string; location: string; onDeleted?: () => void; onUpdated?: () => void }) {
  ...
  return <section className="ui-card overflow-hidden"><div className="grid lg:grid-cols-[minmax(15rem,0.75fr)_1.25fr]"><div className="relative min-h-60 bg-surface-elevated">{heroImage ? <Image alt={`Zdjęcie główne: ${title}`} className="object-cover" fill sizes="(max-width: 1024px) 100vw, 42vw" src={heroImage} unoptimized /> : <div className="flex h-full min-h-60 items-center justify-center bg-gradient-to-br from-surface-hover to-surface text-sm text-muted-foreground">Brak zdjęcia głównego</div>}<div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/65 to-transparent" /></div><div className="flex flex-col p-5 sm:p-7"><div className="flex flex-wrap gap-2"><span className="ui-badge border-gold/25 bg-gold/10 text-gold">{sourceLabel(property.source)}</span><span className="ui-badge">{statusLabel(property.status)}</span></div><h1 className="type-page-title mt-4">{title}</h1><p className="mt-2 flex items-center gap-2 text-sm text-muted-foreground"><MapPin className="size-4" />{location || "Lokalizacja nieuzupełniona"}</p><div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3"><InfoMetric label="Cena" value={formatCurrency(property.price)} emphasis /><InfoMetric label="Cena / m²" value={formatCurrency(property.pricePerSqm)} /><InfoMetric label="Ocena inwestycji" value={formatFlipScore(property.flipScore ?? score.score)} emphasis /></div><div className="mt-6 flex flex-wrap gap-2">{property.listingId ? <Button nativeButton={false} render={<Link href={`/deals/${encodeURIComponent(property.listingId)}`} />} size="sm"><BrainCircuit className="size-3.5" />Otwórz Deal Room</Button> : null}{onUpdated ? <PropertyEditDialog property={property} onUpdated={onUpdated} /> : null}{property.originalUrl ? <Button nativeButton={false} render={<a href={property.originalUrl} rel="noreferrer" target="_blank" />} size="sm" variant="outline"><ExternalLink className="size-3.5" />Otwórz ogłoszenie</Button> : null}<Button nativeButton={false} render={<Link href="/properties" />} size="sm" variant="outline"><ArrowLeft className="size-3.5" />Powrót do listy</Button>{onDeleted ? <PropertyDeleteControl propertyId={property.id} onDeleted={onDeleted} /> : null}</div></div></div></section>;
}
...
function InfoMetric({ label, value, emphasis = false }: { label: string; value: string; emphasis?: boolean }) { return <div className={`ui-metric min-w-0 ${emphasis ? "border-primary/20 bg-primary/[0.06]" : ""}`}><p className="text-xs text-muted-foreground">{label}</p><p className="type-financial-standard mt-1">{value}</p></div>; }

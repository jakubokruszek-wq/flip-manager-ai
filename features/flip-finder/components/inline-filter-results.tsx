...
function latestScansText(scans: SearchFilterScan[]): string { const latest = new Map<string, SearchFilterScan>(); for (const scan of scans) { const current = latest.get(scan.source); if (!current || scan.startedAt > current.startedAt) latest.set(scan.source, scan); } return [...latest.values()].map((scan) => `${scan.source === "otodom" ? "Otodom" : scan.source === "olx" ? "OLX" : scan.source === "morizon" ? "Morizon" : "Facebook"}: sprawdzono ${scan.scannedCount}, dopasowano ${scan.matchedCount}`).join(" · "); }

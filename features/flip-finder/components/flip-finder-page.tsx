...
export function FlipFinderPage() {
  ...
  const clearResults = async (filter: SearchFilterListItem) => {
    ...
    try {
      const response = await apiFetch(`/api/flip-finder/search-filters/${filter.id}/clear-results`, { method: "POST", credentials: "same-origin" });
      const payload: unknown = await readJson(response);
...
function scanResponseFromProgress(progress: ScanProgressResponse): ScanResponse {
  ...
  return {
    ...
    matchedCount: collector?.mainFeed.matched ?? progress.totals.matched,
...
function scanFunnel(response: ScanResponse): ScanFunnel {
  ...
  const canonical = isRecord(raw.mainFeed) ? raw.mainFeed as Record<string, unknown> : {};
  ...
  const rent = numberFromAny(canonical, ["rentProperty"], numberFromAny(breakdown, ["rent", "rent_listing"], 0));
  const otherExact = numberFromAny(canonical, ["otherExact"], Math.max(0, exact - sell - rent));

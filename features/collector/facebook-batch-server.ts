...
export type CollectorBatchResult = {
  ...
  /** Sanitized per-post detail for FACEBOOK_FILTER_RECONCILE_FAILED outcomes only (see post-flow.ts). Never changes accounting; purely diagnostic. */
...
export async function processFacebookCollectorBatch(deviceId: string, batch: FacebookCollectorBatch): Promise<CollectorBatchResult> {
  ...
  try {
    ...
    return finishBatch(supabase, deviceId, batchRowId, batch, { status, batchId: batch.batchId, captured: batch.posts.length, processed, listingsCreated, listingsUpdated, skipped, errors, sourceScanIds, health: batch.health, persistenceDiagnostics, accounting, reconciliationDiagnostics });
  } catch (error) {
...
function batchForStorage(batch: FacebookCollectorBatch): Record<string, unknown> {
  return { scanId: batch.scanId, batchId: batch.batchId, sourceId: batch.sourceId, sourceType: batch.sourceType, sourceUrl: batch.sourceUrl, collectedAt: batch.collectedAt, health: batch.health, searchTelemetry: batch.searchTelemetry, sourceTabDiagnostics: batch.sourceTabDiagnostics ?? null, stageTelemetry: batch.stageTelemetry ?? [], imageMode: batch.imageMode, imageNetworkDiagnostics: batch.imageNetworkDiagnostics, mainFeedTelemetry: batch.mainFeedTelemetry ?? [], recall: batch.recall ?? null, posts: batch.posts.map((post) => ({ ...post, textHash: post.text ? createHash("sha256").update(post.text).digest("hex") : null, text: post.text?.slice(0, 20_000) ?? null, media: post.media.map((media) => ({ ...media, url: media.url.slice(0, 2_000) })) })) };
}

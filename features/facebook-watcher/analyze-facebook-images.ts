...
const SCHEMA = {
  ...
  required: ["visibleText", "city", "district", "neighborhood", "street", "price", "area", "rooms", "floor", "totalFloors", "condition", "sellerType", "confidence"],
  properties: {
...
export async function analyzeFacebookImages(images: string[], postText?: string): Promise<VisionExtraction | null> {
  ...
  const validImages = images.map(validHttpsUrl).filter((value): value is string => value !== null).slice(0, 6);
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      instructions: "Odczytaj dane wyłącznie z widocznego tekstu na screenshotach posta Facebook oraz opcjonalnego tekstu użytkownika. Nie zgaduj. Nie wyprowadzaj ceny, adresu ani parametrów z wyglądu mieszkania. Niepewne lub niewidoczne pola zwróć jako null. visibleText ma być wierną transkrypcją istotnej treści ogłoszenia. sellerType=private tylko gdy tekst mówi właściciel, prywatnie, bezpośrednio lub bez pośredników.",
...
function validHttpsUrl(value: string): string | null { try { const url = new URL(value); return url.protocol === "https:" && url.hostname ? url.toString() : null; } catch { return null; } }

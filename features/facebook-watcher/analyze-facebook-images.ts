...
const SCHEMA = {
  ...
  required: ["isProperty", "title", "description", "visibleText", "city", "district", "neighborhood", "street", "price", "area", "rooms", "floor", "totalFloors", "condition", "sellerType", "confidence", "fieldConfidence"],
  properties: {
...
export async function analyzeFacebookImages(images: string[], postText?: string): Promise<VisionExtraction | null> {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      instructions: "Odczytaj dane wyłącznie z głównej treści posta Facebook widocznej na wycinku oraz opcjonalnego, bezpiecznie pozyskanego tekstu. Ignoruj autora, timestamp, reakcje, przyciski i komentarze. Wykonaj dokładny OCR całego regionu posta. Szczególnie szukaj ceny, powierzchni zapisanej jako m²/m2/metraż, liczby pokoi zapisanej słownie, cyfrą lub jako M2/M3/M4, oraz miasta, ulicy, osiedla i dzielnicy. Nie pomijaj pierwszej cyfry ceny i nie zgaduj brakujących cyfr. Nie wyprowadzaj ceny, adresu ani parametrów z samego wyglądu mieszkania. isProperty=true tylko dla oferty sprzedaży lub wynajmu nieruchomości. title i description twórz wyłącznie z widocznej treści ogłoszenia. Niepewne lub niewidoczne pola zwróć jako null. visibleText ma być wierną transkrypcją istotnej treści ogłoszenia. Dla każdego pola podaj niezależne fieldConfidence 0-1; dla null ustaw 0. sellerType=private tylko gdy tekst mówi właściciel, prywatnie, bezpośrednio lub bez pośredników.",
